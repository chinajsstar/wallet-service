...
func ExportAddress(addressDataDir string, uniName string, accs []*types.Account) (error) {
	// online and offline path
	dbOnlinePath := addressDataDir + "/" + GetOnlineUniDBName(uniName)
	dbOfflinePath := addressDataDir + "/" + GetOfflineUniDBName(uniName)
...
func ImportAddress(addressDataDir string, uniDBName string) ([]*types.Account, error) {
	dbPath := addressDataDir + "/" + uniDBName
...
func ImportAddressMap(addressDataDir string, uniDBName string) (map[string]*types.Account, error) {
	dbPath := addressDataDir + "/" + uniDBName
	fmt.Println("正在读取地址信息：", dbPath)
...
// 校验在线文件
func VerifyOnlineDBFile(addressDataDir string, uniName string, oriAccs []*types.Account) (error) {
	uniOnlineDBName := GetOnlineUniDBName(uniName)
	fmt.Printf("校验在线文件: %s\n", uniOnlineDBName)
	aCcsOnline, err := ImportAddress(addressDataDir, uniOnlineDBName)
	if err != nil {
...
// 校验离线文件
func VerifyOfflineDBFile(addressDataDir string, uniName string, oriAccs []*types.Account) (error) {
	...
	fmt.Printf("校验离线文件: %s\n", uniOfflineDBName)
	aCcsOffline, err := ImportAddress(addressDataDir, uniOfflineDBName)
	if err != nil {
