...
//				fmt.Printf("Transfer state changed, from:%d, to %d\n", state, transfer.state)
//			}
//
//			//fmt.Printf("current state is :%d\n", transfer.state)
//
//			switch transfer.state {
//			case wtypes.Tx_state_commited:
//				fmt.Printf("trnasaction was mined on block number:%d", transfer.blocknumber.Uint64())
//			case wtypes.Tx_state_confirmed:
//				if uint16(transfer.confirmed_number)<transfer.confirmation_number {
...
//				}
//			case wtypes.Tx_state_unconfirmed:
...
//		}
//
//		if transfer.state==wtypes.Tx_state_confirmed && uint16(transfer.confirmed_number)>=transfer.confirmation_number {
//			fmt.Printf("transaction: %s, hash: 0x%x, is confirmed! ", transfer.identifer, transfer.tx.Hash())
//			transferchan<- transfer
//			break
//		} else if transfer.state==wtypes.Tx_state_unconfirmed {
//			fmt.Printf("transaction: %s, hash: 0x%x, is unconfirmed!!!", transfer.identifer,
//				transfer.tx.Hash())
//			break
//		}
//
...
//	client, err := eth.NewClient("ws://127.0.0.1:8500")
//	if err!=nil {
//		t.Log("error:", err)
//		return
//	}
...
//	wlts := tks.Wallets()
//	t.Logf("there are %d blockchain_server in keystore\n", len(wlts))
//	t.Log("------------wallets accounts information------------")
...
//	t.Logf("there are %d accounts in blockchain_server\n", len(accs))
//	//passpharse := "ko2005,./123eth"
//
//	for _, ac := range accs {
